Create a single horizontal sprite strip for the Codex app digital pet `sus` in the state `running`.
...
Output exactly 6 separate animation frames arranged left-to-right in one single row. Each frame must show the same pet: Small chibi cream cat with oversized glossy dark eyes, peach nose, rounded cheeks, upright ears, and a slightly suspicious curious expression..
